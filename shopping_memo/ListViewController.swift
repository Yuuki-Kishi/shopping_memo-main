...
class ListViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        if listArray.isEmpty { noCellLabel.isHidden = false }
        ...
        if connect {
            self.ref.child("rooms").child(self.roomIdString).child("members").observeSingleEvent(of: .value, with: { [self] snapshot in
                ...
                if memberCount == 1 {
                    ...
                    let alert: UIAlertController = UIAlertController(title: "削除できません", message: "このルームにあなた以外のユーザーがいます。", preferredStyle: .alert)
                ...
                alert.addTextField { textField in
                    ...
                    alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { action in
                        if alertTextField.text != "" {
                            ...
                            GeneralPurpose.updateEditHistory(roomId: self.roomIdString, listId: listId)
                        }}))
                    self.present(alert, animated: true, completion: nil)
                }
                // 実行結果に関わらず記述
            ...
            alert.addTextField { textField in
                ...
                alert.addAction(UIAlertAction(title: "新規作成", style: .default, handler: { action in
                    if textField.text != "" {
                        ...
                        let listId = "list\(date)"
                        self.ref.child("rooms").child(self.roomIdString).child("lists").child(listId).child("info").updateChildValues(["listName": text, "listCount": -1])
                        GeneralPurpose.updateEditHistory(roomId: self.roomIdString, listId: listId)
                        textField.text = ""
                    }}))}
            self.present(alert, animated: true, completion: nil)
