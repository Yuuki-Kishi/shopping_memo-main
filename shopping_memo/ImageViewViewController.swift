...
class ImageViewViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func observeRealtimeDatabase() {
        ...
        ref.child("rooms").child(roomIdString).child("lists").child(listIdString).child("memo").observe(.childChanged, with: { snapshot in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "toCVC" {
            ...
            let view = nav.viewControllers[nav.viewControllers.count - 1] as! CameraViewController
            view.roomIdString = self.roomIdString!
            view.listIdString = self.listIdString!
            view.memoIdString = self.memoIdString!
            view.shoppingMemoName = self.shoppingMemoName!
        }
    }
}
